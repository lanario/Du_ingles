...
/**
 * Correção da tarefa pelo professor, a partir do planejador. `getOrgAssignmentById`
 * roda com service-role e só confere a escola — a turma ser dele é o que
 * autoriza a página, e por isso vem antes das leituras de gabarito e entregas.
 */
export default async function ProfessorTarefaPage({ params }: PageProps) {
  const { id } = await params;
  const ctx = await requireRole(["teacher"]);

  const assignment = await getOrgAssignmentById(id, ctx.organizationId);
  if (!assignment) notFound();
  if (!(await isGroupOwnedByTeacher(assignment.groupId, ctx.userId))) notFound();
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8 px-4 py-8 sm:px-6">
      <header>
        <BackLink
          fallbackHref="/professor/planejador"
      ...
      <section className="space-y-4">
        ...
        {/* `variant="teacher"`: a correção se autoriza pelo RLS de dono da
            turma, não pelo recorte de escola do admin. */}
        <SubmissionReview
          assignmentId={id}
