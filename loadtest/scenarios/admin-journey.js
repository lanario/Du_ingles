...
export function runAdminJourney(baseUrl) {
  let res = http.get(`${baseUrl}/admin`, { tags: { page: "painel" } });
  expectAuthedPage(res, "admin:painel");
  sleep(thinkTime());

  res = http.get(`${baseUrl}/admin/alunos`, { tags: { page: "alunos" } });
  ...
  if (turmaId) {
    res = http.get(`${baseUrl}/admin/turmas/${turmaId}`, { tags: { page: "turma-detalhe" } });
  ...
  if (userId) {
    res = http.get(`${baseUrl}/admin/usuarios/${userId}`, { tags: { page: "usuario-detalhe" } });
    expectAuthedPage(res, "admin:usuario-detalhe");
    sleep(thinkTime());
  }

  res = http.get(`${baseUrl}/admin/financeiro`, { tags: { page: "financeiro" } });
  expectAuthedPage(res, "admin:financeiro");
  sleep(thinkTime());

  res = http.get(`${baseUrl}/admin/relatorios`, { tags: { page: "relatorios" } });
  expectAuthedPage(res, "admin:relatorios");
  sleep(thinkTime());

  res = http.get(`${baseUrl}/admin/mensagens`, { tags: { page: "mensagens" } });
